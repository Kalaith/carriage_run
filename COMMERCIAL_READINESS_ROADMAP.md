...
| Retention | Expedition pays flat gold, no reward variety, no meta-progression, nothing persists. |
...
- [ ] **Escalation cap / finale**: a boss-leg or defined run length (e.g. 8 legs + finale) so runs have an arc and a win condition, not just "leave or die".
- [ ] **Meta-progression**: expedition currency or unlock track that persists (new starting relics, cosmetics, expedition-only guard variants). Currently expeditions are session-only and never saved (`journey.rs:1-9`).
- [ ] **Seeded/daily runs**: the deterministic `SeededRng` architecture (`mission.rs:165-174`) makes a shareable daily-run trivially cheap — a strong streamer feature for the stated target audience.
- [ ] **Run summary & records**: best-run stats, run history screen, per-run seed display.
