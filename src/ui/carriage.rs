...
pub(super) fn draw_carriage(assets: &AssetManager, run: &MissionRun) {
    ...
    let id = match visual.chassis_slots { 0..=2 => "scout_cart", 3 => "merchant_wagon", _ => "heavy_wagon" };
    let tint = if run.carriage.hit_flash.finished() { WHITE } else { Color::new(1.0, 0.82, 0.68, 1.0) };
    ...
    for (active, upgrade) in [
        ...
        if active {
            draw_world(assets, upgrade, vec2(x, run.carriage.pos.y + 48.0), vec2(26.0, 26.0), WHITE);
            x += 19.0;
        }
    }
}
