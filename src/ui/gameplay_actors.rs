...
pub(super) fn draw_enemy(assets: &AssetManager, enemy: &Enemy) {
    let tint = if enemy.hit_flash.finished() { WHITE } else { Color::new(1.0, 0.72, 0.72, 1.0) };
    let size = if matches!(enemy.kind, EnemyKind::AlphaWolf | EnemyKind::ArmoredBandit) { 94.0 } else { 82.0 };
    draw_character(assets, enemy_id(enemy.kind), enemy.pos, vec2(size, size), tint);
    draw_health_bar(vec2(enemy.pos.x - 26.0, enemy.pos.y - enemy.radius - 18.0), 52.0, enemy.health, enemy.max_health, Color::new(0.78, 0.18, 0.18, 1.0));
}
...
pub(super) fn draw_guard(assets: &AssetManager, guard: &Guard) {
    let tint = if !guard.is_active() { Color::new(0.45, 0.48, 0.50, 0.62) } else if !guard.hit_flash.finished() { Color::new(0.76, 0.90, 1.0, 1.0) } else { WHITE };
    draw_character(assets, guard.kind.id(), guard.pos, vec2(82.0, 82.0), tint);
    draw_health_bar(vec2(guard.pos.x - 28.0, guard.pos.y + 30.0), 56.0, guard.health, guard.max_health, Color::new(0.22, 0.68, 0.88, 1.0));
}

pub(super) fn draw_guard_icon(assets: &AssetManager, kind: GuardKind, pos: Vec2, enabled: bool) {
    let tint = if enabled { WHITE } else { Color::new(0.55, 0.55, 0.55, 0.48) };
...
pub(super) fn draw_shot(assets: &AssetManager, shot: &Shot) {
    ...
    let id = if shot.color.b > shot.color.r { "magic_bolt" } else { "arrow" };
